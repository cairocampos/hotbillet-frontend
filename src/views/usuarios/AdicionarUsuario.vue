  ...
  <div>
    ...
    <form class="md:flex justify-between form-sm px-10">
      <div class="space-y-12">
          ...
          <InputGroup label="Senha" classCustom="border-b border-gray-400" labelClass="text-xs text-default">
            <template #input>
              <input :type="showPassword ? 'text' : 'password'" v-model="user.password" class="w-100 bg-transparent outline-none" />
            </template>

            <template #prepend>
              <button type="button" class="align-middle" @click="generatePassword">
                <span class="text-xs text-default">Gerar Senha</span>                    
              </button>
            </template>
            <template #append>
              <button type="button" class="align-middle" @click="showPassword = !showPassword">
          ...
          <div class="form-group" v-if="user.user_profile_id == PROFILES.VENDEDOR">
